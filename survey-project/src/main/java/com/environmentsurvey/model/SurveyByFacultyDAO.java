...
public class SurveyByFacultyDAO {
	public boolean addSurvey(SurveyStudentBean bean) throws SQLException {
		Connection con = DBConnect.getConn();
		String sql = "INSERT INTO E_ADDSURVEYBYFACULTY (ID,NAME,DESCRIPTION,VOLUNTEER,DATE,VENUE) VALUES (NULL,?,?,?,?,?)";
	...
	public static int update(SurveyStudentBean bean) throws SQLException
	{
		int status=0;
		Connection con=DBConnect.getConn();
		String sql="UPDATE E_ADDSURVEYBYFACULTY SET NAME=?,DESCRIPTION=?,VOLUNTEER=?,DATE=?,VENUE=? WHERE ID=?";
		PreparedStatement preparedStatement=con.prepareStatement(sql);
		preparedStatement.setString(1, bean.getName());
		preparedStatement.setString(2, bean.getDescription());
		preparedStatement.setString(3, bean.getVolunteers());
	...
	public static SurveyStudentBean getRecordById(int id) throws SQLException {
		...
		Connection con=DBConnect.getConn();
		String sql="SELECT * FROM E_ADDSURVEYBYFACULTY WHERE ID=?";
		PreparedStatement preparedStatement=con.prepareStatement(sql);
	...
	public static int delete(int id) throws SQLException {
		...
		String sql="DELETE FROM E_ADDSURVEYBYFACULTY WHERE ID=?";
